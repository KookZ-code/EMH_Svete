  ...
  function exportHtml() {
    ...
    const lossRow = `<div style="display:flex;gap:8px;padding:10px 24px;background:#F8FAFC;border-bottom:1px solid #e0e0e0">
      ...
      <div style="display:flex;gap:8px;flex:1">
        ...
        ${lc2(Math.max(0,Math.round(((kpi?.sbo_pct??0)-lossStats.idle.pct)*10)/10)+'%','SBO','#00897B',`${lossStats.sbo.events} ev · MTTR ${fmtMtx(lossStats.sbo.mttr)}`,`Total ${fmtMtx(lossStats.sbo.totalMin)}`)}
    ...
    // ── Heatmap HTML (same generator as DA) ──────────────────────────────────
    const hmHtml = (() => {
      if (!heatmap) return '';
      ...
      const rowHtml = hmRows.map(row => {
        ...
        const mttw_s = mttw > 0 ? ` <span style="font-size:8px;color:rgb(${row.baseRgb});background:rgba(0,0,0,.06);border-radius:3px;padding:1px 4px">⏱ ${fmtMtx(mttw)}</span>` : '';
        ...
        const evCells = row.hours.map(b => {
          const a=intensity(b.ev,rmax); return `<div style="width:${COL_W}px;height:${EV_H}px;background:rgba(${row.baseRgb},${a});color:${tc(a)};display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:700;border-radius:4px;flex-shrink:0">${b.ev||''}</div>`;
        }).join('') + `<div style="width:${TOT_W}px;padding-right:6px;font-size:11px;font-weight:800;color:rgb(${row.baseRgb});display:flex;align-items:center;justify-content:flex-end;flex-shrink:0">${evTot} ev</div>`;
        const stCells = row.state.length ? row.state.map(cnt => {
          const a=Math.round((cnt/smax*0.75+0.10)*100)/100; return `<div style="width:${COL_W}px;height:${SUB_H}px;background:rgba(${row.baseRgb},${a});color:${tc(a)};display:flex;align-items:center;justify-content:center;font-size:10px;font-weight:700;border-radius:3px;border:1px dashed rgba(0,0,0,.08);flex-shrink:0">${cnt||''}</div>`;
        }).join('') + `<div style="width:${TOT_W}px;padding-right:6px;font-size:9px;font-weight:800;color:rgb(${row.baseRgb});display:flex;align-items:center;justify-content:flex-end;flex-shrink:0">peak ${Math.max(...row.state)}</div>` : '';
        return `<div style="display:flex;gap:3px;margin-bottom:2px;align-items:center">
          <div style="width:${LBL_W}px;flex-shrink:0;display:flex;flex-direction:column;gap:1px;padding:2px 4px 2px 8px;border-left:3px solid rgb(${row.baseRgb})">
            ...
            <span style="font-size:8px;color:#94A3B8">events started${mttw_s}</span>
          </div>${evCells}</div>
          ${row.state.length ? `<div style="display:flex;gap:3px;margin-bottom:4px;align-items:center">
            <div style="width:${LBL_W}px;flex-shrink:0;padding:2px 4px 2px 12px;border-left:2px dashed rgb(${row.baseRgb})"><span style="font-size:8px;font-weight:700;color:#94A3B8">in state</span></div>${stCells}</div>` : ''}`;
      }).join('');
      const nonRunCells = totalState.map(cnt => {
        const pct=cnt/(machines.length||1)*100; const a=Math.round((pct/100*0.8+0.08)*100)/100;
        return `<div style="width:${COL_W}px;height:${SUB_H+2}px;background:rgba(15,23,42,${a});color:${tc(a)};display:flex;align-items:center;justify-content:center;font-size:10px;font-weight:700;border-radius:4px;flex-shrink:0">${cnt||''}</div>`;
      }).join('') + `<div style="width:${TOT_W}px;padding-right:6px;font-size:9px;font-weight:800;color:#0F172A;display:flex;flex-direction:column;align-items:flex-end;justify-content:center;flex-shrink:0">peak <b style="color:#CC0000">${Math.max(...totalState)}</b></div>`;
      const sumCells = shiftHours.map((_,i) => `<div style="width:${COL_W}px;text-align:center;font-size:10px;font-weight:600;color:#64748B;padding:4px 0;border-top:2px solid #E2E8F0;flex-shrink:0">${hmRows.reduce((s,r)=>s+r.hours[i].ev,0)||'—'}</div>`).join('')
        + `<div style="width:${TOT_W}px;text-align:right;padding:4px 6px 0;font-size:11px;font-weight:800;color:#0F172A;border-top:2px solid #E2E8F0;flex-shrink:0">${hmRows.reduce((s,r)=>s+r.hours.reduce((a,b)=>a+b.ev,0),0)} ev</div>`;
      return `<div style="background:#fff;border:1px solid #E2E8F0;border-radius:8px;padding:16px 20px;margin:0 24px 16px;overflow-x:auto">
        ...
        <div style="min-width:${LBL_W+shiftHours.length*(COL_W+3)+3+TOT_W}px">
          ...
          <div style="display:flex;gap:3px;margin-top:2px;align-items:center">
            <div style="width:${LBL_W}px;flex-shrink:0;padding:2px 4px 2px 8px;border-left:3px solid #0F172A;margin-top:2px"><span style="font-size:9px;font-weight:700;color:#0F172A">Non-running total</span></div>${nonRunCells}</div>
