  ...
  function exportHtml() {
    ...
    const lossRow = `<div style="display:flex;gap:8px;padding:10px 24px;background:#F8FAFC;border-bottom:1px solid #e0e0e0">
      ...
      <div style="display:flex;gap:8px;flex:1">
        ...
        ${lc(Math.max(0,Math.round((kpi.sbo_pct-lossStats.idle.pct)*10)/10)+'%','SBO','#00897B',`${lossStats.sbo.events} ev · MTTR ${fmtMtx(lossStats.sbo.mttr)}`,`Total ${fmtMtx(lossStats.sbo.totalMin)}`)}
